    ...
    <div className="space-y-4">
      {comments.length === 0 ? (
        <p className="flex items-center gap-2 text-xs text-text-subtle">
          ...
          No comments yet. Use this to ask the requester a question without rejecting the request.
        </p>
      ) : (
        <ul className="space-y-3">
          {comments.map((c) => (
            <li key={c.id} className="flex gap-2.5">
              <Avatar name={c.authorName ?? 'System'} size="sm" className="mt-0.5" />
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-baseline gap-x-2">
                  <span className="text-xs font-medium text-text">{c.authorName ?? 'System'}</span>
                  {c.authorType === 'AI' && (
                    <span className="rounded bg-accent-soft px-1 text-[10px] font-medium text-accent">AI</span>
                  )}
                  <span className="text-[11px] text-text-subtle tabular">{formatDateTime(c.createdAt)}</span>
                </p>
      ...
      {canComment && (
        <form onSubmit={submit} className="space-y-2">
          ...
          <Textarea
            ...
            rows={3}
            maxLength={4000}
            placeholder="Add a comment. The requester is notified."
            disabled={pending}
          />
          {error && (
