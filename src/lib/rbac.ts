...
export function requestVisibility(session: SessionUser): SQL | undefined {
  if (can(session, 'request.viewAll')) return undefined; // no restriction
  ...
  return or(...(clauses.filter(Boolean) as SQL[]));
}
...
export function visibilitySql(session: SessionUser, alias = 'requests'): SQL {
  if (can(session, 'request.viewAll')) return sql`true`;
...
export function canViewRequest(
  session: SessionUser,
  row: { requesterId: string; departmentId: string | null; requestType: string },
  approverIds: string[] = [],
): boolean {
