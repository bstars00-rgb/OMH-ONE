 ...
 */
export async function getOrCreateReview(requestId: string, opts: { force?: boolean } = {}): Promise<FullReview | null> {
  ...
  try {
    summary = await provider.summarizeRequest(ctx);
    policy = await provider.reviewPolicy(ctx);
    risk = await provider.detectRisk(ctx, policy);
  } catch (err) {
    ...
    summary = await fallback.summarizeRequest(ctx);
    policy = await fallback.reviewPolicy(ctx);
    risk = await fallback.detectRisk(ctx, policy);
    degraded = true;
  }
  ...
  if (opts.force) await db.delete(aiReviews).where(eq(aiReviews.requestId, requestId));
 ...
 */
export async function backfillAiReviews(limit = 60) {
  try {
    const db = await ready();
    const pending = await db
      ...
      .where(
        sql`${requests.status} in ('SUBMITTED','IN_REVIEW')
            and not exists (select 1 from ai_reviews ar where ar.request_id = ${requests.id})`,
      )
      .limit(limit);

    if (pending.length === 0) return;
    console.log(`[ai] generating reviews for ${pending.length} open request(s)`);
    for (const row of pending) {
      try {
        await getOrCreateReview(row.id);
      } catch (err) {
